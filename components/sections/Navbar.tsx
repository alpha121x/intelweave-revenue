        ...
        <nav className="hidden lg:flex items-center space-x-8">
          {pages.map((page, index) => (
            ...
            >
              {page.label}
              <span
                className={`absolute -bottom-1 left-0 h-0.5 bg-gradient-to-r from-primary to-white transition-all duration-300 ${
                  pathname === page.path ? "w-full" : "w-0 group-hover:w-full"
                }`}
              ></span>
            </Link>
          ))}
